...
레퍼런스 타입: 배열에 대한, 클래스에 대한, 인터페이스에 대한


>>셤문제: 생성자의 특징 세가지를 쓰시오
  클래스 이름과 동일한 특별한 메소드
  객체가 생성될 떄 자동으로 한 번 생성되는 메소드
  개발자는 객체를 초기화하는 데 필요한 코드 작성
  객체가 생성될 때 반드시 호출됨 (그러므로 하나 이상 선언되어야 함)
  
>>셤문제: 다형성이 뭐임?
	같은 이름의 메소드가 클래스 등에 따라 다르게 동작하도록 구현하는 것
>>셤문제: 자바에서 사용하는 접근지정자 네개랑 의미 쓰기
  private default protected public
    x      o         o       o     같은패키지 클래스 접근여부
    x      x         x       o     다른         "
